...
(defn unwrap-meta [x]
  (walk/postwalk
   (fn [node]
     (if (:com.fulcrologic.guardrails-analyzer/meta-wrapper? node)
       (:value node)
       node))
   x))
...
(defn fix-kw-nss [x]
  (let [gr-ns   (namespace ::gr.reg/_)
        this-ns (namespace ::this)]
    (walk/postwalk
     (fn [node]
       (if (and (qualified-keyword? node)
                (= gr-ns (namespace node)))
         (keyword this-ns (name node))
         node))
     x)))
