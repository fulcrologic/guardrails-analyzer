(ns com.fulcrologic.guardrails-analyzer.analysis2.purity
  (:require
   [com.fulcrologic.guardrails-analyzer.analysis.analyzer.dispatch :as d]
   [com.fulcrologic.guardrails-analyzer.analysis.purity-data :as purity-data]
   [com.fulcrologic.guardrails-analyzer.analysis.sampler :as cp.sampler]
...
(defn- scan-arity-purity
  "ONE pass over `(::cp.art/arities fn-detail)` collecting all purity
   signals from gspec metadata. Returns a map:

     {:pure?     boolean  ;; any arity carries :pure?/:pure (after shorthand conversion)
      :pure-mock fn-or-nil ;; first :pure-mock encountered, if any}

   Used by `pure-function?`, `has-pure-mock?`, and `get-pure-mock` so the
   arity map is iterated only once per call site (instead of three times)."
  [fn-detail]
  (reduce-kv
   (fn [acc _arity arity-detail]
     (if-let [gspec (::cp.art/gspec arity-detail)]
       (let [metadata (cp.sampler/convert-shorthand-metadata
                       (::cp.art/metadata gspec))]
         (cond-> acc
           (and (not (:pure-mock acc)) (contains? metadata :pure-mock))
           (assoc :pure-mock (:pure-mock metadata))
           (or (get metadata ::cp.sampler/pure)
               (get metadata :pure?))
           (assoc :pure? true)))
       acc))
   {:pure? false :pure-mock nil}
   (::cp.art/arities fn-detail)))

(defn- registry-purity
  "Resolve `fn-sym` against the function/external registries and return its
   consolidated purity signals via `scan-arity-purity`, or nil if not
   registered."
  [env fn-sym]
  (when-let [fn-detail (or (cp.art/function-detail env fn-sym)
                           (cp.art/external-function-detail env fn-sym))]
    (scan-arity-purity fn-detail)))
...
or is a known-pure clojure.core function."
      ...
      ;; Check metadata in function registry
         (:pure? (registry-purity env fn-sym)))))
...
Pure mocks allow us to evaluate functions during analysis without side effects."
       [env fn-sym]
       [::cp.art/env symbol? => boolean?]
       (boolean (:pure-mock (registry-purity env fn-sym))))
...
Returns the mock function or nil if not available."
       ...
       (:pure-mock (registry-purity env fn-sym)))
