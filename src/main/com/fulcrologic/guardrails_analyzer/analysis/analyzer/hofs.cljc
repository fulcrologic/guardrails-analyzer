...
(ns com.fulcrologic.guardrails-analyzer.analysis.analyzer.hofs
  (:require
...
(defn- transform-gspecs [td keep-keys]
  (update td ::cp.art/arities
          (fn [arities]
            (reduce-kv
             (fn [m k v]
               (assoc m k
                      (if (::cp.art/gspec v)
                        (update v ::cp.art/gspec select-keys keep-keys)
                        v)))
             {}
             arities))))
...
(defn >fn-ret [td]
  (transform-gspecs
   ...
   [::cp.art/return-spec ::cp.art/return-type ::cp.art/return-predicates]))

(defn >fn-args [td]
  (transform-gspecs
   (select-keys td [::cp.art/arities])
   [::cp.art/argument-specs ::cp.art/argument-types ::cp.art/argument-predicates]))
