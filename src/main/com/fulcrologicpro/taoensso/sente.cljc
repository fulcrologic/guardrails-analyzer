(ns com.fulcrologicpro.taoensso.sente
  ...
  #?(:clj
     (:require
      ...
      [com.fulcrologicpro.taoensso.encore :as enc :refer (swap-in! reset-in! swapped )]
      [com.fulcrologicpro.taoensso.timbre :as timbre :refer (tracef debugf infof warnf errorf)]
      [com.fulcrologicpro.taoensso.sente.interfaces :as interfaces]))

  #?(:cljs
     (:require
      [clojure.string :as str]
      [cljs.core.async :as async :refer (<! >! put! chan)]
      [com.fulcrologicpro.taoensso.encore :as enc :refer (format swap-in! reset-in! swapped) ]
      [com.fulcrologicpro.taoensso.timbre :as timbre :refer-macros (tracef debugf infof warnf errorf)]
      [com.fulcrologicpro.taoensso.sente.interfaces :as interfaces]))

  #?(:cljs
     (:require-macros
      [cljs.core.async.macros :as asyncm :refer (go go-loop)]
      [com.fulcrologicpro.taoensso.sente :as sente-macros :refer (elide-require)]))

  #?(:clj
     (:import
       [com.fulcrologicpro.org.java_websocket.client WebSocketClient]
       (com.fulcrologicpro.org.java_websocket.handshake ServerHandshake)
       (com.fulcrologicpro.org.java_websocket.framing CloseFrame)
       (java.net URI)
       (java.util Map))))

(if (vector? com.fulcrologicpro.taoensso.encore/encore-version)
...
(comment
  (do
    (require '[com.fulcrologicpro.taoensso.sente.packers.transit :as transit])
...
(defn make-channel-socket-server!
  ...
  [1] e.g. `(com.fulcrologicpro.taoensso.sente.server-adapters.http-kit/get-sch-adapter)` or
           `(com.fulcrologicpro.taoensso.sente.server-adapters.immutant/get-sch-adapter)`.
  ...
  (let [allowed-origins allowed-origins
...
#?(:cljs (def ajax-lite "Alias of `com.fulcrologicpro.taoensso.encore/ajax-lite`" enc/ajax-lite))
...
#?(:clj
   (defn- create-java-client-websocket!
     ...
     (let [uri (URI. uri-str)
           ...
           ws-client
           (proxy [WebSocketClient] [^URI uri ^Map headers]
             (onOpen    [^ServerHandshake handshakedata] nil)
    ...
    (let [{?timeout-ms :timeout-ms ?cb :cb :keys [flush?]} opts
      ...
      (if-not (:open? @state_) ; Definitely closed
        ...
        (let [?cb-uuid (when ?cb-fn (enc/uuid-str 6))
          ...
          (when-let [cb-uuid ?cb-uuid]
            (reset-in! cbs-waiting_ [cb-uuid] ?cb-fn)
...
#?(:cljs
   ...
   (defn make-channel-socket-client!
     ...
     Common options:
       ...
       :packer         ; :edn (default), or an IPacker implementation.
       :ajax-opts      ; Base opts map provided to `com.fulcrologicpro.taoensso.encore/ajax-lite`.
