(ns com.fulcrologicpro.taoensso.timbre
  ...
  #?(:clj
     (:require
      ...
      [com.fulcrologicpro.taoensso.encore    :as enc :refer [ qb]]
      [com.fulcrologicpro.taoensso.timbre.appenders.core :as core-appenders])
  ...
  #?(:cljs
     (:require-macros
      [com.fulcrologicpro.taoensso.timbre])))
...
(def default-config
  "Default/example Timbre `*config*` value:
    ...
    {:min-level :debug #_[[\"com.fulcrologicpro.taoensso.*\" :error] [\"*\" :debug]]
     :ns-filter #{\"*\"} #_{:deny #{\"com.fulcrologicpro.taoensso.*\"} :allow #{\"*\"}}
...
(enc/defonce ^:dynamic *config*
  ...
  MAIN OPTIONS
    ...
    :min-level
      ...
      `[[#{\"com.fulcrologicpro.taoensso.*\"} :error] ... [{\"*\"} :debug]]`.
  ...
  COMPILE-TIME LEVEL/NS ELISION
    To control :min-level and :ns-filter at compile-time, use:
      ...
      - `com.fulcrologicpro.taoensso.timbre.min-level.edn`  JVM property (read as edn)
      - `com.fulcrologicpro.taoensso.timbre.ns-pattern.edn` JVM property (read as edn)
...
(comment
  (enc/qb 1e6 ; [145.78 275.69]
    (may-log-ns? "*" "com.fulcrologicpro.taoensso.timbre")
    (ns->?min-level [[#{"com.fulcrologicpro.taoensso.*" "foo.bar"} :info] ["*" :debug]] "foo.bar")
...
#?(:clj
   (def ^:private compile-time-min-level
     (when-let [level
                (or
                  (enc/read-sys-val "com.fulcrologicpro.taoensso.timbre.min-level.edn" "TAOENSSO_TIMBRE_MIN_LEVEL_EDN")
...
#?(:clj
   (def ^:private compile-time-ns-filter
     (let [ns-pattern
           (or
             (enc/read-sys-val "com.fulcrologicpro.taoensso.timbre.ns-pattern.edn" "TAOENSSO_TIMBRE_NS_PATTERN_EDN")
  ...
  (let [{:keys [?ns-str] :or {?ns-str (str *ns*)}} opts]
    ...
    (when-not #?(:clj (-elide? level ?ns-str) :cljs false)
      (let [{:keys [config ?err ?file ?line ?base-data spying?]
             :or   {config 'com.fulcrologicpro.taoensso.timbre/*config*
...
#?(:clj
   (defn refer-timbre
     "Shorthand for:
     (require '[com.fulcrologicpro.taoensso.timbre :as timbre
...
#?(:clj
   (def ^:private default-stacktrace-fonts
     (or
       (enc/read-sys-val "com.fulcrologicpro.taoensso.timbre.default-stacktrace-fonts.edn" "TAOENSSO_TIMBRE_DEFAULT_STACKTRACE_FONTS_EDN")
