(ns com.fulcrologicpro.fulcro.mutations
  ...
  * One or more definitions of what to do when the mutation is requested.
  ...
  "
  #?(:cljs (:require-macros com.fulcrologicpro.fulcro.mutations))
  (:require
    [com.fulcrologicpro.fulcro.components :as comp]
    [com.fulcrologicpro.fulcro.dom.events :as evt]
...
(>def ::env (s/keys :req-un [:com.fulcrologicpro.fulcro.application/app]))
...
#?(:cljs
   (com.fulcrologicpro.fulcro.mutations/defmutation set-props
     "
...
#?(:cljs
   (com.fulcrologicpro.fulcro.mutations/defmutation toggle
...
(>defn with-response-type
  ...
  :text, and :document."
  [env response-type]
  [::env :com.fulcrologicpro.fulcro.networking.http-remote/response-type => ::env]
  (assoc-in env [:ast :params :com.fulcrologicpro.fulcro.networking.http-remote/response-type] response-type))
   ...
   (defn defmutation* [macro-env args]
     (let [conform!       (fn [element spec value]
                            (when-not (s/valid? spec value)
                              (throw (IllegalArgumentException. (str "Syntax error in " element ": " (s/explain-str spec value)))))
                              ...
                              :result-action    (fn [~'env]
                                                  (binding [comp/*after-render* true]
                                                    (when-let [~'default-action (ah/app-algorithm (:app ~'env) :default-result-action!)]
                                                      (~'default-action ~'env))))})
           doc            (or doc "")
           multimethod    `(defmethod com.fulcrologicpro.fulcro.mutations/mutate '~fqsym [~env-symbol]
                             (let [~(first arglist) (-> ~env-symbol :ast :params)]
                               ~method-map))]
       (if (= fqsym sym)
         multimethod
         `(do
            (def ~(with-meta sym {:doc doc}) (com.fulcrologicpro.fulcro.mutations/->Mutation '~fqsym))
            ~multimethod)))))
