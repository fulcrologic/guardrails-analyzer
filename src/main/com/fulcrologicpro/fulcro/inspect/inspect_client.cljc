(ns ^:no-doc com.fulcrologicpro.fulcro.inspect.inspect-client
  "Functions used by Fulcro to talk to Fulcro Inspect."
  #?(:cljs (:require-macros com.fulcrologicpro.fulcro.inspect.inspect-client))
  (:require
    [com.fulcrologicpro.edn-query-language.core :as eql]
    [com.fulcrologicpro.fulcro.algorithms.lookup :as ah]
    #?@(:cljs [[goog.object :as gobj]
               [com.fulcrologicpro.fulcro.inspect.diff :as diff]
               [com.fulcrologicpro.fulcro.inspect.transit :as encode]
               [cljs.core.async :as async]])
    [clojure.data :as data]
    [com.fulcrologicpro.taoensso.encore :as encore]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn app-state [app] (some-> app :com.fulcrologicpro.fulcro.application/state-atom deref))
(defn runtime-atom [app] (some-> app :com.fulcrologicpro.fulcro.application/runtime-atom))
(defn state-atom [app] (some-> app :com.fulcrologicpro.fulcro.application/state-atom))
(defn app-uuid [app] (some-> app :com.fulcrologicpro.fulcro.application/state-atom deref (get app-uuid-key)))
(defn remotes [app] (some-> (runtime-atom app) deref :com.fulcrologicpro.fulcro.application/remotes))
(defn app-id [app] (some-> (app-state app) :fulcro.inspect.core/app-id))
(defn fulcro-app-id [app] (:com.fulcrologicpro.fulcro.application/id app))
  ...
  #?(:cljs
     (case type
       ...
       (encore/if-let [{:fulcro.inspect.core/keys [app-uuid]} data
                       ...
                       historical-app   (assoc app :com.fulcrologicpro.fulcro.application/state-atom (atom historical-state))
       ...
       (let [{:keys                          [query mutation]
         ...
         (encore/when-let [app       (get @apps* app-uuid)
           ...
           (transmit! remote {:com.fulcrologicpro.fulcro.algorithms.tx-processing/id             tx-id
                              :com.fulcrologicpro.fulcro.algorithms.tx-processing/ast            ast
                              :com.fulcrologicpro.fulcro.algorithms.tx-processing/idx            0
                              :com.fulcrologicpro.fulcro.algorithms.tx-processing/options        {}
                              :com.fulcrologicpro.fulcro.algorithms.tx-processing/update-handler identity
                              :com.fulcrologicpro.fulcro.algorithms.tx-processing/result-handler (fn [{:keys [body] :as result}]
...
(defn optimistic-action-finished!
  ...
  [app
   {:keys [component ref state com.fulcrologicpro.fulcro.algorithms.tx-processing/options]}
  ...
  #?(:cljs
     (let [component-name (get-component-name component)
           ...
           tx             (cond-> {:fulcro.inspect.ui.transactions/tx-id                    tx-id
                                   ...
                                   :com.fulcrologicpro.fulcro.algorithms.tx-processing/options options}
...
(defmacro ido
  ...
  goog.DEBUG and com.fulcrologicpro.fulcro.inspect.inspect-client/INSPECT are false.

  This allows you to enable inspect messages in production by adding the following to
  your compiler config:

  :closure-defines {\"com.fulcrologicpro.fulcro.inspect.inspect_client.INSPECT\" true}
  "
  [& body]
  (when (cljs? &env)
    `(when (and (or ~'goog.DEBUG INSPECT) (not= "disabled" INSPECT))
