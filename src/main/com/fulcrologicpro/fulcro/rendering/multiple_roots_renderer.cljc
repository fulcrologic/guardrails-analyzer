(ns com.fulcrologicpro.fulcro.rendering.multiple-roots-renderer
  ...
  ```
  "
  #?(:cljs (:require-macros [com.fulcrologicpro.fulcro.rendering.multiple-roots-renderer :refer [with-app-context]]))
  (:require
    [com.fulcrologicpro.fulcro.rendering.keyframe-render :as kr]
    [com.fulcrologicpro.fulcro.rendering.ident-optimized-render :as ior]
   ...
   (let [app (or app (comp/any->app react-instance) comp/*app*)]
     (if (map? app)
       (let [class             (comp/react-type react-instance)
             ...
             known-roots       (some-> app :com.fulcrologicpro.fulcro.application/runtime-atom deref ::known-roots)
         ...
         (swap! (:com.fulcrologicpro.fulcro.application/runtime-atom app) update-in [::known-roots k] (fnil conj #{}) react-instance))
   ...
   (let [app (or app (comp/any->app react-instance) comp/*app*)]
     (if (map? app)
       (let [class (comp/react-type react-instance)
             k     (comp/class->registry-key class)]
         (log/debug "Adding root of type " k)
         (swap! (:com.fulcrologicpro.fulcro.application/runtime-atom app) update-in [::known-roots k] disj react-instance))
       (log/error "Deregister-root cannot find app. Pass your Fulcro app via options.")))))

(defn render-roots! [app options]
  (let [state-map   (some-> app :com.fulcrologicpro.fulcro.application/state-atom deref)
        known-roots (some-> app :com.fulcrologicpro.fulcro.application/runtime-atom deref ::known-roots)]
...
(defn render-stale-components!
  ...
  [app options]
  (let [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom]} app
        {:com.fulcrologicpro.fulcro.application/keys [only-refresh]} @runtime-atom
                                     ...
                                     (this-as ^js this
                                       (let [js-props (.-props this)]
                                         (with-app-context fulcro-app
                                           (binding [fdn/*denormalize-time* (-> fulcro-app :com.fulcrologicpro.fulcro.application/runtime-atom deref ::basis-t)]
                                             (let [query     (comp/get-query UIRoot)
                                                   state-map (some-> fulcro-app :com.fulcrologicpro.fulcro.application/state-atom deref)
...
(defn floating-root-factory
  ...
  ([UIClass options]
   (let [constructor     (fn [])
         ...
         render          (fn [this]
                           (binding [fdn/*denormalize-time* (-> comp/*app* :com.fulcrologicpro.fulcro.application/runtime-atom deref ::basis-t)]
                             (let [state-map (some-> comp/*app* :com.fulcrologicpro.fulcro.application/state-atom deref)
