(ns com.fulcrologicpro.fulcro.react.hooks
  ...
  #?(:cljs
     (:require-macros [com.fulcrologicpro.fulcro.react.hooks :refer [use-effect use-lifecycle]]))
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.algorithms.normalized-state :as fns]
...
(defn use-gc
  ...
  (use-lifecycle
    nil
    (fn []
      (let [state (-> this-or-app comp/any->app :com.fulcrologicpro.fulcro.application/state-atom)]
        (swap! state fns/remove-entity ident edges)))))

(let [initial-mount-state (fn []
                            (let [componentName (keyword "com.fulcrologicpro.fulcro.floating-root" (gensym "generated-root"))]
  ...
  (defn use-fulcro-mount
    ...
    (let [st                 (useState initial-mount-state)
          ...
          _                  (use-lifecycle
                               ...
                               (fn []
                                 (let [join-key (aget key-and-root 0)
                                       state    (-> parent-this comp/any->app :com.fulcrologicpro.fulcro.application/state-atom)]
