(ns com.fulcrologicpro.fulcro.offline.load-cache
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.algorithms.do-not-use :as dnu]
    [com.fulcrologicpro.fulcro.algorithms.scheduling :as scheduling]
    [com.fulcrologicpro.fulcro.mutations :as m]
    [com.fulcrologicpro.fulcro.data-fetch :as df]
    [com.fulcrologicpro.fulcro.algorithms.do-not-use]
...
(defn current-load-store
  ...
  [app]
  (get-in app [:com.fulcrologicpro.fulcro.application/config ::load-cache]))

(defn load-store-options
  "Returns the options that were passed when configuring the load cache."
  [app]
  (get-in app [:com.fulcrologicpro.fulcro.application/config ::options]))
...
(defn with-load-cache
  ...
  * `::merge-new-result?` - Boolean. Only applies if `::eager?` was set to true: should the real network result
  ...
  ([app edn-store options]
   (-> app
     (assoc-in [:com.fulcrologicpro.fulcro.application/config :load-mutation] `internal-load!)
     (assoc-in [:com.fulcrologicpro.fulcro.application/config ::load-cache] edn-store)
     (assoc-in [:com.fulcrologicpro.fulcro.application/config ::options] options)))
  ([app edn-store]
