(ns com.fulcrologicpro.fulcro.offline.durable-mutations
   ...
   * Order-independent
   ...
   Usage: Use `transact!` from this ns, or add `:com.fulcrologicpro.fulcro.offline.durable-mutations/durable? true` to the
   options of your call to `comp/transact!`. Do not include follow-on reads in the transaction if using `comp/transact!`.
   "
  (:require
    [clojure.core.async :as async]
    #?@(:cljs [[com.fulcrologicpro.fulcro.algorithms.tempid :as tempid :refer [TempId]]]
        :clj  [[com.fulcrologicpro.fulcro.algorithms.tempid :as tempid]])
    [com.fulcrologicpro.fulcro.algorithms.tx-processing :as txn]
    [com.fulcrologicpro.fulcro.application :as app]
    [com.fulcrologicpro.taoensso.timbre :as log]
    [com.fulcrologicpro.fulcro.offline.durable-edn-store :as des]
    [com.fulcrologicpro.fulcro.offline.tempid-strategy :as tids]
    [com.fulcrologicpro.fulcro.algorithms.lookup :as ah]
    [com.fulcrologicpro.fulcro.algorithms.do-not-use :refer [now]]
...
(defn- with-durable-transact
  ...
  [app]
  (let [normal-transact! (get-in app [::app/algorithms :com.fulcrologicpro.fulcro.algorithm/tx!])]
    (log/debug "Installing write-through transact support")
    (-> app
      (assoc-in [::app/algorithms ::original-tx!] normal-transact!)
      (assoc-in [::app/algorithms :com.fulcrologicpro.fulcro.algorithm/tx!]
...
(defn- with-augmented-result-action [app]
  (let [result-action! (get-in app [::app/algorithms :com.fulcrologicpro.fulcro.algorithm/default-result-action!])]
    (assoc-in app [::app/algorithms :com.fulcrologicpro.fulcro.algorithm/default-result-action!]
