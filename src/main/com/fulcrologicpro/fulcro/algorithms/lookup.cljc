(ns com.fulcrologicpro.fulcro.algorithms.lookup
  "Fulcro is quite customizable, and all of the pluggable algorithms are stored on the app. This
  very easily leads to a desire to alias the long com.fulcrologicpro.fulcro.application namespace
...
(defn app-algorithm
  ...
  "
  [{:com.fulcrologicpro.fulcro.application/keys [algorithms] :as app} k]
  (when-let [nm (when (or (string? k) (keyword? k) (symbol? k))
                  (keyword "com.fulcrologicpro.fulcro.algorithm" (name k)))]
    (get-in app [:com.fulcrologicpro.fulcro.application/algorithms nm] nil)))
