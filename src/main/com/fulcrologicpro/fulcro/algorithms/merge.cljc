(ns com.fulcrologicpro.fulcro.algorithms.merge
...
(defn merge!
  ...
  ([app data-tree query options]
   (let [{:com.fulcrologicpro.fulcro.application/keys [state-atom]} (comp/any->app app)]
     (when state-atom
  ...
  * `named-parameter`: Post-processing ident integration steps. see `targeting/integrate-ident*`. You may also
  ...
  (when-let [app (comp/any->app app)]
    (if-not (comp/has-ident? component)
      ...
      (let [state   (:com.fulcrologicpro.fulcro.application/state-atom app)
