(ns com.fulcrologicpro.fulcro.algorithms.indexing
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.mutations :refer [defmutation]]
...
(defn index-root!
  ...
  (let [{:com.fulcrologicpro.fulcro.application/keys [state-atom runtime-atom]} app
        {:com.fulcrologicpro.fulcro.application/keys [root-class]} @runtime-atom
    ...
    (swap! runtime-atom (fn [s]
                          (-> s
                            (assoc-in [:com.fulcrologicpro.fulcro.application/indexes :root-props] root-props)
                            (assoc-in [:com.fulcrologicpro.fulcro.application/indexes :linked-props] linked-props)
                            (assoc-in [:com.fulcrologicpro.fulcro.application/indexes :idents-in-joins] idents-in-joins)
                            (assoc-in [:com.fulcrologicpro.fulcro.application/indexes :prop->classes] prop->classes))))))

(defn- index-component* [runtime-state instance ident cls]
  (let [k (comp/class->registry-key cls)]
    (cond-> runtime-state
      k (update-in
          [:com.fulcrologicpro.fulcro.application/indexes :class->components k]
          (fnil conj #{})
          instance)
      ident (update-in
              [:com.fulcrologicpro.fulcro.application/indexes :ident->components ident]
              (fnil conj #{})
              instance))))
...
(defn index-component!
  ...
  [this]
  (let [{:keys [:com.fulcrologicpro.fulcro.application/runtime-atom]} (comp/any->app this)
...
(defn- drop-component*
  ...
  (let [k (comp/class->registry-key cls)]
    (cond-> (update-in runtime-state [:com.fulcrologicpro.fulcro.application/indexes :class->components k]
              disj instance)
      ident (update-in
              [:com.fulcrologicpro.fulcro.application/indexes :ident->components ident]
              disj
