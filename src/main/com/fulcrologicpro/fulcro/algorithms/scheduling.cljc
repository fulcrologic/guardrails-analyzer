(ns com.fulcrologicpro.fulcro.algorithms.scheduling
...
(defn schedule!
  ...
  ([app scheduled-key action tm]
   [:com.fulcrologicpro.fulcro.application/app keyword? fn? int? => any?]
  ...
  ([app scheduled-key action]
   [:com.fulcrologicpro.fulcro.application/app keyword? fn? => any?]
...
(let [raf #?(:clj #(defer % 16)
  ...
  (defn schedule-animation!
    ...
    ([app scheduled-key action]
     [:com.fulcrologicpro.fulcro.application/app keyword? fn? => any?]
     #?(:clj  (action)
        :cljs (let [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom]} app]
                (when-not (get @runtime-atom scheduled-key)
                  (swap! runtime-atom assoc scheduled-key true)
                  (let [f (fn []
                            (swap! runtime-atom assoc scheduled-key false)
