(ns com.fulcrologicpro.fulcro.algorithms.tx-processing
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.algorithms.lookup :as ah]
    [com.fulcrologicpro.fulcro.algorithms.tempid :as tempid]
    [com.fulcrologicpro.fulcro.algorithms.denormalize :as fdn]
    [com.fulcrologicpro.fulcro.algorithms.do-not-use :as futil]
    [com.fulcrologicpro.fulcro.algorithms.scheduling :as sched :refer [schedule!]]
    [com.fulcrologicpro.fulcro.mutations :as m]
    [com.fulcrologicpro.fulcro.components :as comp]
    [com.fulcrologicpro.fulcro.specs]
    [com.fulcrologicpro.fulcro.inspect.inspect-client :as inspect :refer [ido ilet]]
    com.fulcrologicpro.fulcro.specs
...
(defn app->remotes
  ...
  [app]
  [:com.fulcrologicpro.fulcro.application/app => :com.fulcrologicpro.fulcro.application/remotes]
  (-> app :com.fulcrologicpro.fulcro.application/runtime-atom deref :com.fulcrologicpro.fulcro.application/remotes))

(defn app->remote-names
  "Returns a set of the names of the remotes from an app"
  [app]
  [:com.fulcrologicpro.fulcro.application/app => :com.fulcrologicpro.fulcro.application/remote-names]
  (-> app :com.fulcrologicpro.fulcro.application/runtime-atom deref :com.fulcrologicpro.fulcro.application/remotes keys set))
...
(defn top-keys
  ...
  [::ast => (s/coll-of :com.fulcrologicpro.edn-query-language.ast/key)]
  (if (= :root type)
...
(defn combine-sends
  ...
  [:com.fulcrologicpro.fulcro.application/app :com.fulcrologicpro.fulcro.application/remote-name ::send-queue => (s/keys :opt [::send-node] :req [::send-queue])]
...
(defn net-send!
  ...
  [:com.fulcrologicpro.fulcro.application/app ::send-node :com.fulcrologicpro.fulcro.application/remote-name => any?]
...
(defn process-send-queues!
  ...
  [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom] :as app}]
  [:com.fulcrologicpro.fulcro.application/app => ::send-queues]
...
(defn build-env
  ([app {::keys [options] :as tx-node} addl]
   [:com.fulcrologicpro.fulcro.application/app ::tx-node map? => map?]
   (let [{:keys [ref component]} options]
     (cond-> (merge addl {:state (-> app :com.fulcrologicpro.fulcro.application/state-atom)
                          :app   app})
       options (assoc ::options options)
       ref (assoc :ref ref)
       component (assoc :component component))))
  ([app {:keys [::options] :as tx-node}]
   [:com.fulcrologicpro.fulcro.application/app ::tx-node => map?]
...
(defn application-rendered!
  ...
  [{:keys [:com.fulcrologicpro.fulcro.application/runtime-atom] :as app} options]
...
(defn activate-submissions!
  ...
  [{:keys [:com.fulcrologicpro.fulcro.application/runtime-atom] :as app}]
  [:com.fulcrologicpro.fulcro.application/app => any?]
...
(defn schedule-activation!
  ...
  ([app tm]
   [:com.fulcrologicpro.fulcro.application/app int? => any?]
...
(defn advance-actions!
  ...
  [:com.fulcrologicpro.fulcro.application/app ::tx-node => ::tx-node]
                       ...
                       (fn [{:keys [done? new-elements] :as acc} element]
                         (if done?
                           ...
                           (let [{::keys [complete? dispatch original-ast-node idx]} element
                                 ...
                                 state           (:com.fulcrologicpro.fulcro.application/state-atom app)
                                 state-before    @state
...
(defn fully-complete?
  ...
  [:com.fulcrologicpro.fulcro.application/app ::tx-node => boolean?]
...
(defn remove-send!
  ...
  [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom] :as app} remote txn-id ele-idx]
  [:com.fulcrologicpro.fulcro.application/app :com.fulcrologicpro.fulcro.application/remote-name ::id ::idx => any?]
...
(defn record-result!
  ...
  ([{:com.fulcrologicpro.fulcro.application/keys [runtime-atom] :as app} txn-id ele-idx remote result result-key]
   [:com.fulcrologicpro.fulcro.application/app ::id int? keyword? any? keyword? => any?]
  ...
  ([app txn-id ele-idx remote result]
   [:com.fulcrologicpro.fulcro.application/app ::id int? keyword? any? => any?]
...
(defn compute-desired-ast-node
  ...
  [:com.fulcrologicpro.fulcro.application/app :com.fulcrologicpro.fulcro.application/remote-name ::tx-node ::tx-element => ::tx-element]
...
(defn add-send!
  ...
  [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom] :as app} {::keys [id options] :as tx-node} ele-idx remote]
  [:com.fulcrologicpro.fulcro.application/app ::tx-node ::idx :com.fulcrologicpro.fulcro.application/remote-name
   => (s/nilable ::send-node)]
...
(defn queue-element-sends!
  ...
  [:com.fulcrologicpro.fulcro.application/app ::tx-node ::tx-element => ::tx-node]
...
(defn element-with-work
  ...
  [:com.fulcrologicpro.fulcro.application/remote-names ::tx-element => (s/nilable ::tx-element)]
...
(defn dispatch-result!
  ...
  [:com.fulcrologicpro.fulcro.application/app ::tx-node ::tx-element keyword? => ::tx-element]
  (schedule-queue-processing! app 0)
...
(defn distribute-element-results!
  ...
  [:com.fulcrologicpro.fulcro.application/app ::tx-node ::tx-element => ::tx-element]
  (reduce
...
(defn process-tx-node!
  [app {:keys [::options] :as tx-node}]
  [:com.fulcrologicpro.fulcro.application/app ::tx-node => (s/nilable ::tx-node)]
  (let [optimistic? (boolean (:optimistic? options))]
...
(defn requested-refreshes [app queue]
  [:com.fulcrologicpro.fulcro.application/app (s/coll-of ::tx-node) => set?]
...
(defn remotes-active-on-node
  ...
  [::tx-node :com.fulcrologicpro.fulcro.application/remote-names
   => :com.fulcrologicpro.fulcro.application/remote-names]
...
(defn active-remotes
  ...
  [queue remotes]
  [::active-queue :com.fulcrologicpro.fulcro.application/remote-names
   => :com.fulcrologicpro.fulcro.application/active-remotes]
  (reduce
    (fn [ra n]
...
(defn process-queue!
  ...
  [{:com.fulcrologicpro.fulcro.application/keys [state-atom runtime-atom] :as app}]
  [:com.fulcrologicpro.fulcro.application/app => any?]
  (let [new-queue        (reduce
    ...
    (swap! state-atom assoc :com.fulcrologicpro.fulcro.application/active-remotes remotes-active?)
    (swap! runtime-atom assoc ::active-queue new-queue)
    (when (seq explicit-refresh)
      (swap! runtime-atom update :com.fulcrologicpro.fulcro.application/to-refresh accumulate explicit-refresh))
    (schedule-render! app)
    nil))
  ...
  (let [mutation-nodes      (:children (eql/query->ast tx))
    ...
    (when mixed-tx-node
      (let [node         (run-actions! app mixed-tx-node)
            runtime-atom (:com.fulcrologicpro.fulcro.application/runtime-atom app)]
...
(defn default-tx!
  ...
  "
  ([app tx]
   [:com.fulcrologicpro.fulcro.application/app ::tx => ::id]
   (default-tx! app tx {}))
  ([{:com.fulcrologicpro.fulcro.application/keys [runtime-atom] :as app} tx {:keys [synchronous?] :as options}]
   [:com.fulcrologicpro.fulcro.application/app ::tx ::options => ::id]
   (if synchronous?
     ...
     (do
       ...
       (let [{:keys [refresh only-refresh ref] :as options} (merge {:optimistic? true} options)
         ...
         (swap! runtime-atom (fn [s] (cond-> (update s ::submission-queue (fn [v n] (conj (vec v) n)) node)
                                       ...
                                       (seq refresh) (update :com.fulcrologicpro.fulcro.application/to-refresh accumulate refresh)
                                       (seq only-refresh) (update :com.fulcrologicpro.fulcro.application/only-refresh accumulate only-refresh))))
         (::id node))))))
...
(defn abort!
  ...
  [app abort-id]
  (let [{:com.fulcrologicpro.fulcro.application/keys [runtime-atom]} (comp/any->app app)
        runtime-state   @runtime-atom
        {:com.fulcrologicpro.fulcro.application/keys [remotes]
...
(defn abort-remote!
  ...
  (let [app            (comp/any->app app-ish)
        {:com.fulcrologicpro.fulcro.application/keys [state-atom runtime-atom]} (comp/any->app app)
        {abort-network! :abort!
         :as            the-remote} (get @runtime-atom [:com.fulcrologicpro.fulcro.application/remotes remote])
        old-send-queue (get-in @runtime-atom [::send-queues remote])]
    (swap! runtime-atom assoc-in [::send-queues remote] [])
    (swap! state-atom update :com.fulcrologicpro.fulcro.application/active-remotes (fnil disj #{}) remote)
