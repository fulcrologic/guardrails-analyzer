(ns com.fulcrologicpro.fulcro.data-fetch
...
(defn load-params*
  ...
  (let [state-map         (-> app :com.fulcrologicpro.fulcro.application/state-atom deref)
  ...
  (let [env (assoc env :load-params params)]
    (if (fn? ok-action)
      ...
      (let [{:keys [body transaction]} result
            ...
            {:com.fulcrologicpro.fulcro.application/keys [state-atom]} app]
...
(defn load!
  ...
  ([app-or-comp server-property-or-ident class-or-factory config]
   (let [app           (comp/any->app app-or-comp)
         ...
         {:keys [load-marker-default query-transform-default load-mutation]} (-> app :com.fulcrologicpro.fulcro.application/config)
