(ns com.fulcrologicpro.fulcro.components
  #?(:cljs (:require-macros com.fulcrologicpro.fulcro.components))
  (:require
    #?@(:cljs
        [[goog.object :as gobj]
         [cljsjs.react]])
    ...
    [com.fulcrologicpro.fulcro.algorithms.do-not-use :as util]
...
(defn ^:deprecated use-effect
  "DEPRECATED: use from com.fulcrologicpro.fulcro.react.hooks
...
(defn any->app
  ...
  [x]
  (letfn [(fulcro-app? [x] (and (map? x) (contains? x :com.fulcrologicpro.fulcro.application/state-atom)))]
    (cond
...
(defn shared
  ...
  ([comp-or-app]
   (shared comp-or-app []))
  ([comp-or-app k-or-ks]
   (let [shared (or *shared* (some-> (any->app comp-or-app) :com.fulcrologicpro.fulcro.application/runtime-atom deref :com.fulcrologicpro.fulcro.application/shared-props))
         ks     (cond-> k-or-ks
                  (not (sequential? k-or-ks)) vector)]
     (cond-> shared
  ...
  ([class props]
   (if-let [id (ident class props)]
     (do
       ...
       (if (= :com.fulcrologicpro.fulcro.algorithms.merge/not-found (second id)) [(first id) nil] id))
     (when #?(:clj false :cljs goog.DEBUG)
...
(defn get-query
  ...
  ([class-or-factory] (get-query class-or-factory (or *query-state*
                                                    (some-> *app* :com.fulcrologicpro.fulcro.application/state-atom deref) {})))
...
(defn set-query!
  ...
  * `opts` : A map with `query` and optionally `params` (substitutions on queries)
  ...
  (let [app        (any->app x)
        state-atom (:com.fulcrologicpro.fulcro.application/state-atom app)
        queryid    (cond
...
(letfn [(--set-query! [app class-or-factory {:keys [query] :as params}]
          (let [state-atom (:com.fulcrologicpro.fulcro.application/state-atom app)
                queryid    (cond
                             (string? class-or-factory) class-or-factory
                             (some-> class-or-factory meta (contains? :queryid)) (some-> class-or-factory meta :queryid)
                             :otherwise (query-id class-or-factory nil))]
  ...
  (defn refresh-dynamic-queries!
    ...
    ([app-ish]
     (let [{:com.fulcrologicpro.fulcro.application/keys [state-atom] :as app} (any->app app-ish)
...
(defn get-indexes
  ...
  [x]
  (let [app (any->app x)]
    (some-> app :com.fulcrologicpro.fulcro.application/runtime-atom deref :com.fulcrologicpro.fulcro.application/indexes)))
...
(defn component->state-map
  ...
  change over time."
  [this] (some-> this any->app :com.fulcrologicpro.fulcro.application/state-atom deref))
   ...
   (defn- replace-and-validate-fn
     ...
     ([env sym external-args user-arity fn-form user-known-sym]
      (when-not (<= user-arity (count (second fn-form)))
        (throw (IllegalArgumentException. (str "Invalid arity for " user-known-sym ". Expected " user-arity " or more."))))
       ...
       (do
         ...
         (let [to-keyword            (fn [s] (cond
           ...
           (when-let [child-query (some component-query template)]
             (throw (IllegalArgumentException. (str "defsc " class ": `get-query` calls in :query can only be inside a join value, i.e. `{:some/key " child-query "}`"))))
           (when (and (not has-wildcard?) (seq illegal-syms))
             (throw (IllegalArgumentException. (str "defsc " class ": " illegal-syms " was destructured in props, but does not appear in the :query!"))))
   ...
   (defn- build-ident
     ...
     (cond
       keyword (if (is-legal-key? keyword)
                 ...
                 (throw (IllegalArgumentException. (str "The table/id " keyword " of :ident does not appear in your :query"))))
       ...
       template (let [table   (first template)
                  ...
                  (cond
                    (nil? table) (throw (IllegalArgumentException. "TABLE part of ident template was nil" {}))
                    (not (is-legal-key? id-prop)) (throw (IllegalArgumentException. (str "The ID property " id-prop " of :ident does not appear in your :query")))
...
#?(:clj
   (defn- build-render [classsym thissym propsym compsym extended-args-sym body]
     (let [computed-bindings (when compsym `[~compsym (com.fulcrologicpro.fulcro.components/get-computed ~thissym)])
           extended-bindings (when extended-args-sym `[~extended-args-sym (com.fulcrologicpro.fulcro.components/get-extra-props ~thissym)])
           render-fn         (symbol (str "render-" (name classsym)))]
       `(~'fn ~render-fn [~thissym]
          (com.fulcrologicpro.fulcro.components/wrapped-render ~thissym
            (fn []
              (let [~propsym (com.fulcrologicpro.fulcro.components/props ~thissym)
                    ~@computed-bindings
                    ~@extended-bindings]
                ~@body)))))))
     ...
     (let [env           (merge env (meta initial-state))
           ...
           child-state   (fn [k]
                           (let [state-params    (get initial-state k)
                             ...
                             (when code?
                               (throw (IllegalArgumentException. (str "defsc " sym ": Illegal parameters to :initial-state " state-params ". Use a lambda if you want to write code for initial state. Template mode for initial state requires simple maps (or vectors of maps) as parameters to children. See Developer's Guide."))))
                             (cond
                               (not (or from-parameter? to-many? to-one?)) (throw (IllegalArgumentException. (str "Initial value for a child (" k ") must be a map or vector of maps!")))
                               to-one? `(com.fulcrologicpro.fulcro.components/get-initial-state ~child-class ~(parameterized state-params))
                               to-many? (mapv (fn [params]
                                                `(com.fulcrologicpro.fulcro.components/get-initial-state ~child-class ~(parameterized params)))
                                          state-params)
                               from-parameter? `(com.fulcrologicpro.fulcro.components/get-initial-state ~child-class ~(param-expr state-params))
                               :otherwise nil)))
           kv-pairs      (map (fn [k]
                                [k (if (is-child? k)
                                     (child-state k)
       ...
       (when (seq illegal-keys)
         (throw (IllegalArgumentException. (str "Initial state includes keys " illegal-keys ", but they are not in your query."))))
       `(~'fn ~'build-initial-state* [~'params] (com.fulcrologicpro.fulcro.components/make-state-map ~initial-state ~children-by-query-key ~'params)))))
...
#?(:clj
   (defn- build-initial-state [env sym {:keys [template method]} legal-keys query-template-or-method]
     (when (and template (contains? query-template-or-method :method))
       (throw (IllegalArgumentException. (str "When query is a method, initial state MUST be as well."))))
     (cond
...
#?(:clj
   (defn defsc*
     ...
     (when-not (s/valid? ::args args)
       (throw (IllegalArgumentException. (str "Invalid arguments. " (-> (s/explain-data ::args args)
                                                                      ::s/problems
                                                                      first
                                                                      :path) " is invalid."))))
              ...
              (defonce ~(vary-meta sym assoc :doc doc :jsdoc ["@constructor"])
                (fn [props#]
                  (cljs.core/this-as this#
                    ...
                    nil)))
              (com.fulcrologicpro.fulcro.components/configure-component! ~sym ~fqkw options#)))
         ...
         `(do
            ...
            (let [options# ~options-map]
              (def ~(vary-meta sym assoc :doc doc :once true)
                (com.fulcrologicpro.fulcro.components/configure-component! ~(str sym) ~fqkw options#))))))))
     ...
     (try
       ...
       (catch Exception e
         (if (contains? (ex-data e) :tag)
           (throw e)
           (throw (IllegalArgumentException. "Unexpected internal error while processing defsc. Please check your syntax." e)))))))

(defn external-config
  [app-ish k]
  (some-> app-ish (any->app) (get-in [:com.fulcrologicpro.fulcro.application/config :external-config k])))
...
(defn refresh-component!
  ...
  (if (component? component)
    (let [prior-computed (or (get-computed component) {})
          {:com.fulcrologicpro.fulcro.application/keys [state-atom runtime-atom]} (any->app component)
          state-map      @state-atom]
      (swap! runtime-atom update :com.fulcrologicpro.fulcro.application/basis-t inc)
      (binding [fdn/*denormalize-time* (-> @runtime-atom :com.fulcrologicpro.fulcro.application/basis-t)]
