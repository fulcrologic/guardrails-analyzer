(ns com.fulcrologicpro.fulcro.ui-state-machines
  ...
  #?(:cljs (:require-macros com.fulcrologicpro.fulcro.ui-state-machines))
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.algorithms.data-targeting :as targeting]
    [com.fulcrologicpro.fulcro.mutations :as m :refer [defmutation]]
    [com.fulcrologicpro.fulcro.components :as comp]
    [com.fulcrologicpro.fulcro.algorithms.do-not-use :as util :refer [atom?]]
...
(defmutation trigger-state-machine-event
  ...
  (action [{:keys [app] :as env}]
    (let [{::keys [transact-options]} event-data
          {tx-options :com.fulcrologicpro.fulcro.algorithms.tx-processing/options} env
...
(>defn compute-target
  ...
  `:com.fulcrologicpro.fulcro.algorithms.data-targeting/target explicit-target` - A raw Fulcro data fetch target.
...
(>defn trigger-remote-mutation
  ...
  `:com.fulcrologicpro.fulcro.mutations/returning Class` - Class to use for normalizing the result.
  `:com.fulcrologicpro.fulcro.algorithms.data-targeting/target explicit-target` - Target for result
  ...
  `:com.fulcrologicpro.fulcro.algorithms.tx-processing/abort-id` - An abort ID for being able to cancel the mutation.
...
(>defn load-actor
  "Load (refresh) the given actor. If the actor *is not* on the UI, then you *must* specify
   `:com.fulcrologicpro.fulcro.primitives/component-class` in the `options` map.
