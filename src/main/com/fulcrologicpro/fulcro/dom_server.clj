(ns com.fulcrologicpro.fulcro.dom-server
  ...
  (ns app.ui
    (:require
      #?(:clj [com.fulcrologicpro.fulcro.dom-server :as dom] :cljs [com.fulcrologicpro.fulcro.dom :as dom])))"
  (:refer-clojure :exclude [map meta time mask select use set symbol filter])
  (:require
    [com.fulcrologicpro.fulcro.dom-common :as cdom]
    [com.fulcrologicpro.fulcro.components :refer [component-instance?]]
...
(defn- is-element? [e]
  (or
    ...
    (instance? com.fulcrologicpro.fulcro.dom_server.IReactDOMElement e)))
...
(defn- render-to-str* ^StringBuilder [x]
  ...
  (if (vector? x)
    ...
    (let [element ^com.fulcrologicpro.fulcro.dom_server.IReactDOMElement (if-let [element (cond-> x
