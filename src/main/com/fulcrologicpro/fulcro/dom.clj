(ns com.fulcrologicpro.fulcro.dom
  ...
  (ns my-thing.ui
    (:require
      #?(:clj [com.fulcrologicpro.fulcro.dom-server :as dom]
         :cljs [com.fulcrologicpro.fulcro.dom :as dom])))
  ```
  ...
  (:require
    ...
    [com.fulcrologicpro.fulcro.dom-common :as cdom]
...
(defn emit-tag
  ...
  (let [conformed-args      (util/conform! ::dom-macro-args args)
        ...
        create-element      (case str-tag-name
                              "input" 'com.fulcrologicpro.fulcro.dom/macro-create-wrapped-form-element
                              "textarea" 'com.fulcrologicpro.fulcro.dom/macro-create-wrapped-form-element
                              "select" 'com.fulcrologicpro.fulcro.dom/macro-create-wrapped-form-element
                              "option" 'com.fulcrologicpro.fulcro.dom/macro-create-wrapped-form-element
                              'com.fulcrologicpro.fulcro.dom/macro-create-element*)
    ...
    (case attrs-type
      ...
      :runtime-map
      `(com.fulcrologicpro.fulcro.dom/macro-create-element ~str-tag-name ~(into [attrs-value] raw-children) ~css)
...
(defn emit-tag-unwrapped
  ...
  (let [conformed-args         (util/conform! ::dom-macro-args args)
        ...
        attrs-type             (or (first attrs) :nil)      ; attrs omitted == nil
        attrs-value            (or (second attrs) {})
        raw-create-element     'com.fulcrologicpro.fulcro.dom/macro-create-element*
        runtime-create-element 'com.fulcrologicpro.fulcro.dom/macro-create-unwrapped-element
        classes-expression?    (and (= attrs-type :map) (contains? attrs-value :classes))
        attrs-type             (if classes-expression? :runtime-map attrs-type)]
    (case attrs-type
      :js-object                                            ; kw combos not supported
...
(defn- gen-client-dom-fn [create-element-symbol tag]
  `(defn ~tag ~(cdom/gen-docstring tag true)
     [& ~'args]
     (let [conformed-args# (com.fulcrologicpro.fulcro.dom/parse-args ~'args) ; see CLJS file for spec
           {attrs#    :attrs
            children# :children
            css#      :css} conformed-args#
...
(gen-dom-macros com.fulcrologicpro.fulcro.dom/emit-tag com.fulcrologicpro.fulcro.dom/emit-tag-unwrapped)
