(ns com.fulcrologic.guardrails-analyzer.artifacts-spec
  (:require
   [clojure.spec.alpha :as s]
   [clojure.test.check.generators :as gen]
   [com.fulcrologic.guardrails-analyzer.artifacts :as cp.art]
   [com.fulcrologic.guardrails-analyzer.stateful.generators :as st.gen]
...
;; ========== SPECTER-USING FUNCTION TESTS (pre-refactor coverage) ==========

(specification "unwrap-meta"
               (assertions
                "replaces meta-wrapper maps with their :value"
                (cp.art/unwrap-meta {:com.fulcrologic.guardrails-analyzer/meta-wrapper? true
                                     :value                                            42
                                     :kind                                             :number})
                => 42
                ...
                "leaves non-wrapper data unchanged"
                (cp.art/unwrap-meta {:a 1 :b [2 3]})
                => {:a 1 :b [2 3]}

                "handles deeply nested wrappers"
                (cp.art/unwrap-meta {:a {:com.fulcrologic.guardrails-analyzer/meta-wrapper? true
                                         :value                                            "deep"}})
                => {:a "deep"}

                "handles nil"
                (cp.art/unwrap-meta nil)
                => nil
...
(specification "clear-bindings-by-file"
               (let [bindings [{::cp.art/file "a.clj" :data 1}
                               {::cp.art/file "b.clj" :data 2}
                               {::cp.art/file "a.clj" :data 3}
                               {::cp.art/file "c.clj" :data 4}]]
                 (assertions
                  "removes all entries matching the given file"
                  (#'cp.art/clear-bindings-by-file bindings "a.clj")
                  => [{::cp.art/file "b.clj" :data 2}
                      {::cp.art/file "c.clj" :data 4}]

                  "returns all entries when file not found"
                  (#'cp.art/clear-bindings-by-file bindings "z.clj")
                  => bindings

                  "returns empty for empty input"
                  (#'cp.art/clear-bindings-by-file [] "a.clj")
                  => [])))

(specification "clear-problems-by-file"
               (let [problems [{::cp.art/file "a.clj" :problem 1}
                               {::cp.art/file "b.clj" :problem 2}
                               {::cp.art/file "a.clj" :problem 3}]]
                 (assertions
                  "removes all entries matching the given file"
                  (#'cp.art/clear-problems-by-file problems "a.clj")
                  => [{::cp.art/file "b.clj" :problem 2}]

                  "returns all entries when file not found"
                  (#'cp.art/clear-problems-by-file problems "z.clj")
                  => problems

                  "returns empty for empty input"
                  (#'cp.art/clear-problems-by-file [] "a.clj")
                  => [])))
