...
(specification "analyze-apply!" :integration
  (let [env (tf/test-env)]
    (assertions
      (cp.ana/analyze! env
        `(apply + 1 [2 3]))
      =check=> (_/embeds?* {::cp.art/samples #{6}})
      "validates that apply was called correctly"
      (tf/capture-errors cp.ana/analyze! env
        `(apply + 1 2 3))
      =check=>
      (_/seq-matches?*
        [(_/embeds?* {::cp.art/problem-type :error/function-arguments-failed-predicate})])
      ...
      =check=>
      (_/seq-matches?*
        [(_/embeds?* {::cp.art/problem-type :error/function-argument-failed-spec})])
      "validates the collection argument wrt function"
      (tf/capture-errors cp.ana/analyze! env
        `(apply + 1 2 [:kw]))
      =check=>
      (_/seq-matches?*
        [(_/embeds?* {::cp.art/problem-type :error/function-arguments-failed-spec})]))))
...
(specification "analyze-constantly!" :integration
  (let [env (tf/test-env)]
    (assertions
      ...
      =check=> (_/embeds?* {::cp.art/samples #{:ok}}))))
...
(specification "analyze-complement!" :integration
  (let [env (tf/test-env)]
    (assertions
      ...
      =check=> (_/embeds?* {::cp.art/samples #{true}})
      "checks function expected argument count"
      (tf/capture-errors cp.ana/analyze! env
        `((complement symbol?) "str" "bar"))
      =check=>
      (_/seq-matches?*
        [(_/embeds?* {::cp.art/problem-type :error/invalid-function-arguments-count})])
  ...
  (let [env (tf/test-env)]
    (assertions
      ...
      (cp.ana/analyze! env
        `((comp inc str) 0))
      =check=>
      (_/embeds?* {::cp.art/samples (_/every?* (_/is?* number?))}))))
...
(specification "analyze-fnil!" :integration
  (let [env (tf/test-env)]
    (assertions
      (cp.ana/analyze! env
        `((fnil + 1000) 7))
      =check=> (_/embeds?* {::cp.art/samples #{7}})
      ...
      (cp.ana/analyze! env
        `((fnil + "str") 1 2))
      =check=> (_/embeds?* {::cp.art/samples #{3}})
      "nil patches arguments"
      (cp.ana/analyze! env
        `((fnil + 1000) nil))
      =check=> (_/embeds?* {::cp.art/samples #{1000}})
      "fnil can take nil nil-patches"
      (cp.ana/analyze! env
        `((fnil str nil 2 3) nil nil ":three"))
      =check=> (_/embeds?* {::cp.art/samples #{"2:three"}})
  ...
  (let [env (tf/test-env)]
    (assertions
      ...
      (tf/capture-errors cp.ana/analyze! env
        `(partial + "err"))
      =check=> (_/seq-matches?*
                 [(_/embeds?* {::cp.art/problem-type        :error/invalid-partially-applied-arguments
                               ::cp.art/original-expression ["err"]})])
      (cp.ana/analyze! env
        `(partial + "err"))
      =check=> (_/equals?* {})
      ...
      (cp.ana/analyze! env
        `((partial + 1 2 3) 100))
      =check=> (_/embeds?* {::cp.art/samples #{106}})
      (tf/capture-errors cp.ana/analyze! env
        `(partial + 1 2 "err"))
      =check=> (_/seq-matches?*
                 [(_/embeds?* {::cp.art/problem-type :error/invalid-partially-applied-arguments})])
...
(specification "analyze-reduce!" :integration :wip
  (let [env (tf/test-env)
    ...
    (assertions
      ...
      =check=> (_/embeds?* {::cp.art/samples #{10}}))))
...
(specification "analyze-split-with!" :integration :wip
  (let [env (tf/test-env)]
    (assertions
      ...
      =check=> (_/embeds?*
                 {::cp.art/samples #{[[:a :b] [1 2]]}})
      (cp.ana/analyze! env
        `(split-with string? :not-a-coll))
      =check=> (_/embeds?*
                 {::cp.art/samples (_/every?*
                                      (_/is?* vector?)
                                      (tc/of-length?* 2)
                                      (_/every?*
                                        (_/is?* sequential?)))})
...
(specification "analyze-update!" :integration :wip
  (let [env (tf/test-env)]
    (assertions
      (cp.ana/analyze! env
        `(update {:a 0} :a inc))
      =check=> (_/embeds?* {::cp.art/samples #{{:a 1}}})
      (cp.ana/analyze! env
        `(update {:a 1} :a + :kw))
      =check=> (_/embeds?* {::cp.art/samples
