...
=== JVM Mode Requirement: `-Dguardrails.mode=:pro`
...
Without this property, `>defn` expands as a standard Guardrails function and the analyzer cannot
see the spec information it needs to perform checks.

This property must be set regardless of how you run the checker:

* **Separate process** – pass it to the JVM running your checker alias, for example:
+
[source, bash]
-----
clj -J-Dguardrails.mode=:pro -A:dev -X:clj-checker :main-ns checker-startup
-----

* **In-process (same REPL JVM)** – add it to the `:jvm-opts` of your development alias in `deps.edn`:
+
[source]
-----
{:aliases
...
IMPORTANT: This property must be present at JVM *startup*. Setting it later via `System/setProperty`
